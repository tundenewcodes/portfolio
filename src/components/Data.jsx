 export const projectInfo = [{

         host: 'https://facebook-tawny.vercel.app',
             pic1: '../portfolio-images/b-01.jpg',
             pic2: '../portfolio-images/fb-02.jpg',
             pic3: '../portfolio-images/b-03.jpg',
             github: 'https://github.com/tundenewcodes/facebook',
         },
       {
           name: 'Instagram App',
             pic1: 'https://i.ibb.co/SmQ0cMy/instaa.jpg',
             pic2: 'https://i.ibb.co/SmQ0cMy/instaa.jpg',
             pic3: 'https://i.ibb.co/SmQ0cMy/instaa.jpg',
             host: 'https://instagram-nine-iota.vercel.app/',

         github: 'https://github.com/tundenewcodes/instagram',
         },
       {
           name: 'Hulu Video App',

         host: 'https://hulu-clone-two-bay.vercel.app',
             pic1: '../portfolio-images/hu-01.jpg',
             pic3: '../portfolio-images/hu-03.jpg',
             pic2: '../portfolio-images/hu-02.jpg',
             github: 'https://github.com/tundenewcodes/hulu',
         },
       {
           name: 'Google App',

         host: 'https://google-smoky.vercel.app',
             pic1: '../portfolio-images/g0-01.jpg',
             pic2: '../portfolio-images/go-02.jpg',
             pic3: '../portfolio-images/go-03.jpg',
             github: 'https://github.com/tundenewcodes/google',
         },
       {
           name: 'Food Ordering App',

         host: 'https://meal-ordering-app.vercel.app/',
             pic1: '../portfolio-images/fo-01.jpg',
             pic2: '../portfolio-images/fo-2.jpg',
             pic3: '../portfolio-images/fo-3.jpg',
             github: 'https://github.com/tundenewcodes/meal-ordering-app',
         },
       {
           name: 'Burger App',

         host: 'https://my-burger-app-coral.vercel.app',
             pic1: '../portfolio-images/bp-01.jpg',
             pic2: '../portfolio-images/bp-02.jpg',
             pic3: '../portfolio-images/bp-03.jpg',
             github: 'https://github.com/tundenewcodes/myBurger',
         },
       {
           name: 'Dictionary App',

         host: 'https://my-dictionary-one.vercel.app',
             pic1: '../portfolio-images/di-01.jpg',
             pic2: '../portfolio-images/di-02.jpg',
             pic3: '../portfolio-images/di-03.jpg',
             github: 'https://github.com/tundenewcodes/my-dictionary',
         },
       {
           name: 'Rick and monty App',

         host: 'https://rick-and-monty.vercel.app',
             pic1: '../portfolio-images/rm-01.jpg',
             pic2: '../portfolio-images/rm-02.jpg',
             pic3: '../portfolio-images/rm-01.jpg',
             github: 'https://github.com/tundenewcodes/rick-and-monty',
         },
       {
           name: 'Expense Tracker',

         host: 'https://expenses-tracker-webapp.netlify.app',
             pic1: '../portfolio-images/et-01.jpg',
             pic2: '../portfolio-images/et-02.jpg',
             pic3: '../portfolio-images/et-03.jpg',
             github: 'https://github.com/tundenewcodes/expense-tracker',
         },
       {
           name: 'Discord App',

         host: 'https://discord-clone-app.herokuapp.com/',
             pic1: '../portfolio-images/da-01.jpg',
             pic2: '../portfolio-images/da-02.jpg',
             pic3: '../portfolio-images/da-03.jpg',
             github: 'https://github.com/tundenewcodes/discord',
         },
       {
           name: 'Task Management',

         host: 'https://task-manager-web2app.herokuapp.com/',
             pic1: '../portfolio-images/tm-01.jpg',
             pic2: '../portfolio-images/tm-02.jpg',
             pic3: '../portfolio-images/tm-03.jpg',
             github: 'https://github.com/tundenewcodes/node-task-manager',
         },
       {
           name: 'Note App',

         host: 'https://fullstack-note-webapp.herokuapp.com/',
             pic1: '../portfolio-images/ntp-01.jpg',
             pic2: '../portfolio-images/ntp-02.jpg',
             pic3: '../portfolio-images/ntp-03.jpg',
             github: 'https://github.com/tundenewcodes/fullstack-noteapp',
         },
  {
          name: 'E-commerce App',

         host: 'https://mern-ecommerce-full.herokuapp.com/',
         pic1: '../portfolio-images/ms-01.jpg',
           pic2: '../portfolio-images/ms-02.jpg',
           pic3: '../portfolio-images/ms-03.jpg',
         github: 'https://github.com/tundenewcodes/node-ecommerce-app',
        },
       {
           name: 'NASA PLANETS App',

         host: 'https://nasa-planet.herokuapp.com',
             pic1: '../portfolio-images/na-01.jpg',
             pic2: '../portfolio-images/na-02.jpg',
             pic3: '../portfolio-images/na-03.jpg',
             github: 'https://github.com/tundenewcodes/nasa-app',
         },
   ]





 export const toolsData = [{
         name: 'html5,css3 & JS',
         image: 'https://i.ibb.co/BKj0Vh6/trio.jpg',
         level: 'proficient',
     },
     {
         name: 'react',
         image: 'https://i.ibb.co/GtWsPKX/react.png',
         level: 'proficient',
     },
     {
         name: 'redux',
         image: 'https://i.ibb.co/TqYW0b2/redux.png',
         level: 'proficient',
     },
     {
         name: 'nexjs',
         image: 'https://i.ibb.co/Bcjn1gj/next.png',
         level: 'proficient',
     },
     {
         name: 'typescript',
         image: 'https://i.ibb.co/TBYWV95/typescript.png',
         level: 'proficient',
     },
     {
         name: 'tailwind css',
         image: 'https://i.ibb.co/THYcfJ8/tailwind.png',
         level: 'proficient',
     },
     {
         name: 'semantic ui',
         image: 'https://i.ibb.co/Nj66n8C/semantic.png',
         level: 'proficient',
     },

     {
         name: 'python django',
         image: 'https://i.ibb.co/W3T1W1p/pydjango.png',
         level: 'knowledgeable',
     },
     {
         name: 'node + express',
         image: 'https://i.ibb.co/cCz6hZy/nodeex.png',
         level: 'knowledgeable',
     },
     {
         name: 'mysql',
         image: 'https://i.ibb.co/ZWj4DQL/posmysq.jpg',
         level: 'knowledgeable',
     },
     {
         name: 'postgresql',
         image: 'https://i.ibb.co/ZWj4DQL/posmysq.jpg',
         level: 'knowledgeable',
     },
     {
         name: 'graphql',
         image: 'https://i.ibb.co/ZWj4DQL/posmysq.jpg',
         level: 'knowledgeable',
     },
     {
         name: 'graphql',
         image: 'https://i.ibb.co/ZWj4DQL/posmysq.jpg',
         level: 'knowledgeable',
     },
     {
         name: 'mongo db',
         image: 'https://i.ibb.co/GsQz5Z0/mgdb.png',
         level: 'knowledgeable',
     },
     {
         name: 'firebase version 9',
         image: 'https://i.ibb.co/MnPmj8K/fb9.png',
         level: 'knowledgeable',
     },
     {
         name: 'git & github',
         image: 'https://i.ibb.co/wdhXB4P/git-hub.png',
         level: 'knowledgeable',
     },
     {
         name: 'formik & yup',
         image: 'https://i.ibb.co/j5W4xXP/formik.png',
         level: 'knowledgeable',
     },
     {
         name: 'react query',
         image: 'https://i.ibb.co/PTyKGQC/react-query.png',
         level: 'knowledgeable',
     },

 ]